...
class GroupDetailScreen extends StatefulWidget {
  ...
  const GroupDetailScreen({super.key, required this.group, required this.currentUser});
...
class _GroupDetailScreenState extends State<GroupDetailScreen> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(widget.group.name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.gold)),
            Text('${widget.group.members.length} members', style: const TextStyle(fontFamily: 'Cairo', fontSize: 11, color: AppColors.textGreenMuted)),
          ],
        ),
        bottom: TabBar(
          controller: _tabCtrl,
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Send Controlled Alert Reminder:', style: TextStyle(fontFamily: 'Cairo', fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.textDark)),
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: _sending
                      ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(color: AppColors.gold, strokeWidth: 2))
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(message, style: const TextStyle(fontFamily: 'Cairo', fontSize: 13, color: AppColors.textWhite, height: 1.4)),
          const SizedBox(height: 8),
          Text('Pushed by $pusher', style: const TextStyle(fontFamily: 'Cairo', fontSize: 10, color: AppColors.textGreenMuted)),
        ],
      ),
    );
  }
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            if (!isMe)
              Text(name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 10, fontWeight: FontWeight.bold, color: AppColors.primaryMid)),
            const SizedBox(height: 2),
            Text(text, style: TextStyle(fontFamily: 'Cairo', fontSize: 13, color: isMe ? AppColors.textWhite : AppColors.textDark)),
          ],
        ),
      ),
    );
  ...
  void _showPushAlertModal(String type) {
    ...
    final String typeTitle = type == 'namaz' ? 'Namaz Alert' : type == 'tasbeeh' ? 'Tasbeeh Goal' : 'Quran Circle';
    final String typeHint = type == 'namaz' ? 'e.g. Asr prayer is in 10 minutes. Let us pray in congregation!' : type == 'tasbeeh' ? 'e.g. Let us complete 100x Astaghfirullah today.' : 'e.g. Quran recitation circle is active in 1 hour!';
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Push $typeTitle', style: const TextStyle(fontFamily: 'Cairo', fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textDark)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Enter reminder message:', style: TextStyle(fontFamily: 'Cairo', fontSize: 13, color: AppColors.textGrey)),
            const SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(
                color: AppColors.bgWhite,
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: typeHint,
                  hintStyle: const TextStyle(fontFamily: 'Cairo', fontSize: 12, color: AppColors.textLightGrey),
                  border: InputBorder.none,
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('Push Alert', style: TextStyle(fontFamily: 'Cairo', color: AppColors.gold, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(u.name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                        Text(u.email, style: const TextStyle(fontFamily: 'Cairo', fontSize: 11, color: AppColors.textGrey)),
                      ],
                    ),
                  ),
                  FutureBuilder<int>(
                      ...
                      return Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('$s Days', style: const TextStyle(fontFamily: 'Cairo', fontSize: 12, fontWeight: FontWeight.bold, color: AppColors.primaryDark)),
                          ],
                        ),
                      );
                    },
        ...
        children: [
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Group Preferences', style: TextStyle(fontFamily: 'Cairo', fontSize: 15, fontWeight: FontWeight.bold, color: AppColors.textDark)),
                const SizedBox(height: 12),
                _prefSwitch('Mute All Notifications', false),
                _prefSwitch('Namaz Reminder Alerts', widget.group.notifConfig['namaz'] ?? true),
                _prefSwitch('Tasbeeh Reminder Alerts', widget.group.notifConfig['tasbeeh'] ?? true),
                _prefSwitch('Quran Study Alerts', widget.group.notifConfig['quran'] ?? false),
              ],
            ),
          ),
