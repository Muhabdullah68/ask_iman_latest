...
final _kDhikrLibrary = [
  DhikrItem(name: 'SubhanAllah',       arabic: 'سُبْحَانَ اللَّهِ',          meaning: '"Glory be to Allah"',          target: 33),
  DhikrItem(name: 'Alhamdulillah',     arabic: 'الْحَمْدُ لِلَّهِ',          meaning: '"All praise is for Allah"',    target: 33),
  DhikrItem(name: 'Allahu Akbar',      arabic: 'اللَّهُ أَكْبَرُ',           meaning: '"Allah is the Greatest"',      target: 34),
  DhikrItem(name: 'Bismillah',         arabic: 'بِسْمِ اللَّهِ',              meaning: '"In the name of Allah"',       target: 100),
  DhikrItem(name: 'Astaghfirullah',    arabic: 'أَسْتَغْفِرُ اللَّهَ',       meaning: '"I seek forgiveness"',         target: 100),
  DhikrItem(name: 'La ilaha illallah', arabic: 'لَا إِلَٰهَ إِلَّا اللَّهُ', meaning: '"There is no god but Allah"', target: 100),
  DhikrItem(name: 'Salawat',           arabic: 'صَلَّى اللهُ عَلَيْهِ',      meaning: '"Blessings upon the Prophet"', target: 100),
  DhikrItem(name: 'Hasbunallah',       arabic: 'حَسْبُنَا اللَّهُ',           meaning: '"Allah is sufficient for us"', target: 40),
];
...
class _TasbeehScreenState extends State<TasbeehScreen>
  ...
  void _packageTap() {
    ...
    if (newCount >= step.target) {
      ...
      if (_pkgStepIndex < _package.length - 1) {
        ...
        setState(() { _pkgStepCount = step.target; _pkgRunning = false; });
        _triggerCompletion();
      }
    } else {
      ...
      body: Stack(children: [
        SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            if (_packageMode && _package.isNotEmpty) _buildPackageStepsPreview(),
            const SizedBox(height: 40),
          ]),
        ),
        if (_showCompletion)
          _CompletionOverlay(onDismiss: _resetPackage),
      ]),
    );
  }
        ...
        child: Stack(fit: StackFit.expand, children: [
          ...
          const Align(
            ...
            child: Padding(
              ...
              child: Text('Digital Tasbeeh', style: TextStyle(
      ...
      child: Container(
        ...
        child: Row(children: [
          _modePill('Individual', !_packageMode, () {
            setState(() { _packageMode = false; _showCompletion = false; });
          }),
          _modePill('Custom Package', _packageMode, () {
            setState(() { _packageMode = true; _showCompletion = false; });
  ...
  Widget _buildPackageHeader() {
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.add_circle_outline, color: AppColors.primaryDark, size: 22),
              SizedBox(width: 10),
              Text('Build Your Dhikr Package', style: TextStyle(
    ...
    final totalDone = _package.take(_pkgStepIndex).fold(0, (s, e) => s + e.target)
        + _pkgStepCount;
    final totalTarget = _package.fold(0, (s, e) => s + e.target);
    final overall = totalTarget == 0 ? 0.0 : (totalDone / totalTarget).clamp(0.0, 1.0);
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Spacer(),
            Text('$totalDone / $totalTarget', style: const TextStyle(
  ...
  Widget _buildRecitationCard() {
    ...
    return Container(
      ...
      child: Column(children: [
        const Text('CURRENT RECITATION', style: TextStyle(
            fontFamily: 'Cairo', fontSize: 10, fontWeight: FontWeight.w600,
            color: AppColors.gold, letterSpacing: 2.0)),
        const SizedBox(height: 6),
        Text(d.arabic, textDirection: TextDirection.rtl, style: const TextStyle(
            fontFamily: 'Amiri', fontSize: 26,
            color: AppColors.primaryDark, height: 1.8)),
        const SizedBox(height: 2),
        Text(d.name, style: const TextStyle(fontFamily: 'Cairo', fontSize: 22,
            fontWeight: FontWeight.w800, color: AppColors.primaryDark)),
        const SizedBox(height: 2),
        Text(d.meaning, style: const TextStyle(fontFamily: 'Cairo',
          ...
          children: List.generate(_dhikrLibrary.length, (i) {
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: active ? const Color(0xFF8B6914) : const Color(0xFFEEEBE4),
                  borderRadius: BorderRadius.circular(30),
                ),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Text(d.name, style: TextStyle(fontFamily: 'Cairo',
                  ...
                  Container(
                    ...
                    child: Text('${d.target}×', style: TextStyle(fontFamily: 'Cairo',
  ...
  Widget _buildPackageStepsPreview() {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 20),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Package Steps', style: TextStyle(fontFamily: 'Cairo',
            ...
            child: Row(children: [
              Container(
                ...
                child: Center(child: done
                    ...
                    : Text('${i + 1}', style: TextStyle(fontFamily: 'Cairo',
                    fontSize: 12, fontWeight: FontWeight.w700,
                    color: current ? AppColors.primaryDarkest : AppColors.textGrey))),
              ),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(step.dhikr.name, style: TextStyle(fontFamily: 'Cairo',
                    fontSize: 14, fontWeight: FontWeight.w600,
                    color: current ? AppColors.textWhite : AppColors.textDark)),
                Text(step.dhikr.meaning, style: TextStyle(fontFamily: 'Cairo',
                    fontSize: 11,
                    color: current ? AppColors.textGreenMuted : AppColors.textGrey)),
              ])),
              Text(
                current ? '$_pkgStepCount / ${step.target}'
                    : done ? '${step.target} / ${step.target}'
                    : '×${step.target}',
      ...
      builder: (_, scrollCtrl) => Container(
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              const Text('Build Dhikr Package', style: TextStyle(fontFamily: 'Cairo',
                  fontSize: 18, fontWeight: FontWeight.w800, color: AppColors.textDark)),
              const Spacer(),
              GestureDetector(
                onTap: () { widget.onSave(_steps); Navigator.pop(context); },
                child: Container(
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Add to Package', style: TextStyle(fontFamily: 'Cairo',
                  ...
                  children: widget.library.map((d) => GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        ...
                        Text(d.name, style: const TextStyle(fontFamily: 'Cairo',
...
class _StepCard extends StatelessWidget {
  ...
  const _StepCard({
    super.key, required this.step, required this.index,
    required this.onDelete, required this.onTargetChanged,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        const Icon(Icons.drag_handle_rounded, color: AppColors.textLightGrey, size: 20),
        const SizedBox(width: 10),
        Container(
          width: 26, height: 26,
          decoration: BoxDecoration(
        ...
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(step.dhikr.name, style: const TextStyle(fontFamily: 'Cairo',
              fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.textDark)),
          Text(step.dhikr.meaning, style: const TextStyle(fontFamily: 'Cairo',
              fontSize: 11, color: AppColors.textGrey)),
        ])),
        Row(children: [
          _countBtn(Icons.remove, () =>
              onTargetChanged((step.target - 10).clamp(1, 99999))),
          SizedBox(width: 52, child: Center(child: Text('${step.target}',
...
class _CompletionOverlay extends StatefulWidget {
  ...
  @override State<_CompletionOverlay> createState() => _CompletionOverlayState();
}
...
class _CompletionOverlayState extends State<_CompletionOverlay>
  ...
  void initState() {
    ...
    _starRotate = Tween<double>(begin: 0, end: 1)
    ...
    _bgCtrl.forward().then((_) =>
        _starCtrl.forward().then((_) => _textCtrl.forward()));
  }

  @override
  void dispose() {
    _bgCtrl.dispose(); _starCtrl.dispose();
    _textCtrl.dispose(); _particleCtrl.dispose();
    super.dispose();
  }
              ...
              Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Transform.translate(
                  ...
                  child: Opacity(opacity: _textFade.value, child: Column(children: [
                    const Text('مَا شَاءَ اللَّهُ', textDirection: TextDirection.rtl,
                        style: TextStyle(fontFamily: 'Amiri', fontSize: 36,
                            color: AppColors.gold, height: 1.8)),
                    const Text('MashaAllah!', style: TextStyle(fontFamily: 'Cairo',
                        fontSize: 28, fontWeight: FontWeight.w800,
                        color: Colors.white, letterSpacing: 0.5)),
                    const SizedBox(height: 10),
                    const Text('Your dhikr package is complete.',
                        style: TextStyle(fontFamily: 'Cairo', fontSize: 16,
                            color: AppColors.textGreenMuted)),
                    const SizedBox(height: 6),
                    const Text('"And remember Allah much,\nthat you may succeed."',
                        textAlign: TextAlign.center,
                        style: TextStyle(fontFamily: 'Cairo', fontSize: 13,
                            color: AppColors.textGreenMuted,
                            fontStyle: FontStyle.italic, height: 1.5)),
                    const Text('— Quran 8:45', style: TextStyle(fontFamily: 'Cairo',
                    ...
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                            color: AppColors.gold,
                            borderRadius: BorderRadius.circular(28)),
                        child: const Text('Start Again', style: TextStyle(
  ...
  Widget _buildParticle(_Particle p) {
    switch (p.type) {
      case 0:
        return Transform.rotate(angle: math.pi / 4,
            child: Container(width: p.size, height: p.size, color: p.color));
      case 1:
        return Container(width: p.size, height: p.size,
            decoration: BoxDecoration(color: p.color, shape: BoxShape.circle));
      default:
        return SizedBox(width: p.size * 2, height: p.size * 2,
...
class _IslamicStarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawStar(canvas, cx, cy, r * 0.26, r * 0.12, 8, fill: AppColors.primaryDark);
    canvas.drawCircle(Offset(cx, cy), r * 0.06, Paint()..color = AppColors.gold);
  }

  void _drawStar(Canvas c, double cx, double cy, double outer, double inner,
      int pts, {Color? fill}) {
